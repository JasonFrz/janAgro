...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const isProductInOrder = orderCheck.items.some(
        (item) =>
      ...
      if (existingReview) {
        return res.status(400).json({
          success: false,
          message:
            "Anda sudah memberikan review untuk produk ini pada pesanan tersebut.",
        });
      }
      ...
      res
        .status(500)
        .json({ success: false, message: "Server error while adding review." });
    }
  }
);
