...
const LaporanBarangTerlakuCeo = () => {
  ...
  useEffect(() => {
    ...
    if (filterType === "daily") {
      ...
      // Hitung awal minggu (Senin) dan akhir minggu (Minggu) dari tanggal yang dipilih
    ...
    // Dispatch ke Redux/Backend
    dispatch(fetchBestSellingReport({
  ...
  const handleExportPDF = () => {
    ...
    // Judul PDF Dinamis
    let filterTitle = "";
    if(filterType === 'daily') filterTitle = `Harian (${new Date(selectedDate).toLocaleDateString('id-ID')})`;
    else if(filterType === 'monthly') filterTitle = `Bulanan (${selectedMonth}/${selectedYear})`;
            ...
            <div className="flex flex-col md:flex-row justify-between items-end md:items-center mb-6 border-b-2 border-gray-200 pb-4">
                ...
                <div className="flex bg-gray-100 p-1 rounded-md border border-black mt-4 md:mt-0 overflow-x-auto">
                    {[
                        ...
                        { id: 'weekly', label: 'Mingguan', icon: <CalendarDays size={16}/> },
                        { id: 'monthly', label: 'Bulanan', icon: <Calendar size={16}/> },
                        { id: 'yearly', label: 'Tahunan', icon: <Calendar size={16}/> }
                    ].map((type) => (
                        <button
                            key={type.id}
                            onClick={() => setFilterType(type.id)}
                            className={`flex items-center gap-2 px-4 py-2 rounded text-sm font-bold transition-all whitespace-nowrap ${
                                filterType === type.id ? 'bg-black text-white shadow-md' : 'text-gray-600 hover:text-black'
                            }`}
                        >
            ...
            <div className="flex flex-wrap gap-4 items-end">
                ...
                {(filterType === 'daily' || filterType === 'weekly') && (
                    <div className="flex flex-col">
                        <label className="text-xs font-bold uppercase mb-1">Pilih Tanggal</label>
                        <input 
                            type="date" 
                            value={selectedDate}
                        ...
                        />
                        {filterType === 'weekly' && <span className="text-xs text-gray-500 mt-1">*Memilih satu minggu (Senin-Minggu) dari tanggal ini.</span>}
                    </div>
                )}
                ...
                {(filterType === 'monthly') && (
                    <>
                        <div className="flex flex-col">
                            <label className="text-xs font-bold uppercase mb-1">Bulan</label>
                            <select
                                value={selectedMonth}
